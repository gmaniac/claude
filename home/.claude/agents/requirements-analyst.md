...
- **Issue trackers (beads / GitHub MCP)**: Use the `bd` CLI (the project's tracker) or GitHub MCP issue tools (`mcp__github__list_issues`, `search_issues`, `create_issue`) to import existing tickets as input to requirements analysis and to push final user stories back. (No Linear MCP is configured in this environment; discover one via ToolSearch only if it is added later.)
